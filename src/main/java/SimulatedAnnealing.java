public class SimulatedAnnealing extends TSPAlgorithm{
    ...
    public Tour startTour() {
        ...
        while(T > 0.16f && (System.currentTimeMillis() - startTime) < 178000){
            ...
            for(int i=1; i<100; i++){
                ...
                if(getTotDist(candidate) < getTotDist(current)){
                    current = candidate;
                    if(getTotDist(current) < getTotDist(tour)){
                        tour = current;
                    }
                }else if(rand.nextFloat() < Math.pow(Math.E, -((getTotDist(candidate) - getTotDist(current))/T))){
                    current = candidate;
                }
            }
    ...
    private Tour doubleBridge(Tour t){
        int bound = t.size()/4;
